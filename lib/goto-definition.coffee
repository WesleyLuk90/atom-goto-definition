...
module.exports =
    config:
        ...
        ignoreFolders:
            type: 'string'
            default: 'node_modules, build, vendor'
    ...
    getIgnoredPathsGlob: ->
        return ('!' + file.trim() for file in atom.config.get('goto-definition.ignoreFolders').split(','))

    getScanOptions: (options) ->
        options = options || {}
        ...
        scan_paths.push path for path in @getIgnoredPathsGlob()
    ...
    go: (options)->
        {regex, paths} = @getScanOptions(options)
        unless regex
    ...
    getProvider: ->
        return {
            getSuggestionForWord: (textEditor, text, range) =>
                if textEditor.getGrammar().scopeName.indexOf('source.js') > -1
                    return {
                        range,
                        callback: => setTimeout(=>
                            @go({text})
